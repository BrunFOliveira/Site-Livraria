...
<template>
  <header class="bg-white border-b-2 border-(--cor_base_verde) shadow-sm">
    <div class="max-w-7xl mx-auto px-6 py-3 flex items-center gap-8">
      <RouterLink to="/" class="flex items-center gap-3 shrink-0 cursor-pointer select-none">
        <span class="text-black text-xl font-semibold pr-3 border-r-2 border-(--cor_base_verde)">
          IFbooks
        </span>
        <span class="text-(--cor_base_verde) text-xs leading-tight font-medium">
          Apreço à<br />leitura
        </span>
      </RouterLink>

      <div class="flex items-center border border-gray-300 rounded-md overflow-hidden flex-1 max-w-sm">
        <input
          v-model="searchQuery"
          type="text"
          placeholder="Pesquisar"
          class="px-4 py-2 text-sm text-gray-700 placeholder-gray-400 outline-none w-full bg-white"
        />
        <button class="px-3 py-2 text-gray-500 hover:text-(--cor_base_verde) transition-colors bg-white">
      ...
      <nav class="flex items-center gap-7 ml-auto">
      ...
      <div class="flex items-center gap-1 shrink-0">
        <RouterLink to="/cart" class="relative p-2 text-(--cor_base_verde) hover:text-(--cor_base_verde) transition-colors cursor-pointer">
          <img src="/cart.png" alt="Carrinho" class="w-6 h-6" />
          <span v-if="cartCount > 0" class="absolute -top-0.5 -right-0.5 bg-(--cor_base_verde) text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">
            {{ cartCount }}
          </span>
        </RouterLink>
        ...
        <span class="text-gray-300 text-lg select-none">|</span>
        <a href="#" @click.prevent="handleClick('Perfil')" class="p-2 text-(--cor_base_verde) hover:text-(--cor_base_verde) transition-colors cursor-pointer">
          <img src="/user.png" alt="Perfil" class="w-6 h-6" />
        </a>
      </div>
    </div>
  </header>
